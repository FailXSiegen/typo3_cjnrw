        ...
        if (empty($webMounts) === false) {
            ...
            $query->matching(
                $query->logicalAnd(
                    ...
                    $query->logicalOr(
                        $query->in('formPluginUid', $pluginUids ? $pluginUids : [0]),
                        $query->in('siteIdentifier', $pluginUids ? $pluginUids : ['']),
                        $query->logicalAnd(
                            $query->equals('siteIdentifier', ''),
                            $query->equals('pid', 0)
                        )
    ...
    protected function getTreePids(array $webMounts): array
    {
        ...
        if ($webMounts !== null) {
            ...
            foreach ($webMounts as $webMount) {
                $pidsArray = $queryGenerator->getAllSubpageIdentifiers($webMount, 1, 1); //Will be a string like 1,2,3
            }
        }
